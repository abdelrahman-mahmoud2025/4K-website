...
import { ShoppingBag, Users, Award, Globe, MapPin, Phone, Mail, Facebook, Send, User, Map, Clock, Instagram, Sparkles } from 'lucide-react';
    ...
    <div className="container mx-auto px-4 py-16 relative min-h-screen">
      <div className="max-w-6xl mx-auto space-y-20">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
           {[
             { icon: ShoppingBag, color: 'text-primary', val: '3000+', label: t('products_sold'), bg: 'bg-primary/5' },
             { icon: Users, color: 'text-secondary', val: '1500+', label: t('happy_customers_count'), bg: 'bg-secondary/5' },
             { icon: Award, color: 'text-green-500', val: '#1', label: t('top_rated_store'), bg: 'bg-green-500/5' },
             { icon: Clock, color: 'text-blue-500', val: '24/7', label: t('online_support'), bg: 'bg-blue-500/5' },
           ].map((stat, idx) => (
             <div key={idx} className="bg-surface/60 backdrop-blur-sm border border-border p-8 rounded-3xl text-center hover:border-primary/50 transition-all hover:-translate-y-2 shadow-lg group">
                <div className={`${stat.bg} ${stat.color} w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-6 group-hover:scale-110 transition-transform shadow-inner`}>
                  <stat.icon size={32} />
                </div>
                <h3 className="text-text font-black text-3xl mb-2">{stat.val}</h3>
                <p className="text-subtext font-bold text-xs uppercase tracking-widest">{stat.label}</p>
             </div>
           ))}
        </div>
        ...
        <div className="bg-surface/40 backdrop-blur-md rounded-[2.5rem] border border-border p-10 md:p-16 shadow-2xl flex flex-col md:flex-row items-center gap-12 relative overflow-hidden group">
           ...
           <div className="text-center md:text-left relative z-10">
              ...
              <h2 className="text-4xl font-black text-text mb-6 tracking-tight">{t('founder_name')}</h2>
           ...
           <div className="space-y-8">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={idx} className="bg-surface/60 backdrop-blur-sm border border-border p-6 rounded-3xl hover:border-primary/50 transition-all shadow-sm group">
                     <h3 className="text-xl font-bold text-text mb-4 group-hover:text-primary transition-colors">{branch.name}</h3>
                     <div className="space-y-3 text-subtext font-medium">
                        <p className="flex items-center gap-3 bg-background/50 p-3 rounded-xl border border-border/50">
                          <MapPin size={18} className="text-primary" /> {branch.address}
                        </p>
                        <p className="flex items-center gap-3 bg-background/50 p-3 rounded-xl border border-border/50">
                          <Phone size={18} className="text-primary" /> {branch.phone}
                        </p>
                     </div>
                  </div>
                ))}
        ...
        {/* Social Cards */}
        <div className="pt-10">
           <h2 className="text-4xl font-black text-text mb-12 text-center tracking-tight">{t('join_community')}</h2>
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <a href="#" className="bg-[#1877F2] text-white p-10 rounded-[2.5rem] flex flex-col items-center justify-center gap-6 hover:shadow-2xl hover:shadow-[#1877F2]/30 transition-all hover:-translate-y-2 group">
                 ...
                 <div className="text-center">
                   ...
                   <span className="text-white/70 font-bold uppercase tracking-widest text-xs">{t('follow_offers')}</span>
                 </div>
              </a>
              <a href="#" className="bg-[#E4405F] text-white p-10 rounded-[2.5rem] flex flex-col items-center justify-center gap-6 hover:shadow-2xl hover:shadow-[#E4405F]/30 transition-all hover:-translate-y-2 group">
                 <div className="p-5 bg-white/10 rounded-3xl group-hover:scale-110 transition-transform">
                   <Instagram size={48} />
                 </div>
                 <div className="text-center">
                   <span className="text-3xl font-black block mb-1">Instagram</span>
                   <span className="text-white/70 font-bold uppercase tracking-widest text-xs">{t('see_products')}</span>
                 </div>
              </a>
              <a href="#" className="bg-[#25D366] text-white p-10 rounded-[2.5rem] flex flex-col items-center justify-center gap-6 hover:shadow-2xl hover:shadow-[#25D366]/30 transition-all hover:-translate-y-2 group">
                 <div className="p-5 bg-white/10 rounded-3xl group-hover:scale-110 transition-transform">
                   <Phone size={48} />
                 </div>
                 <div className="text-center">
                   <span className="text-3xl font-black block mb-1">WhatsApp</span>
                   <span className="text-white/70 font-bold uppercase tracking-widest text-xs">{t('chat_support')}</span>
                 </div>
              </a>
           </div>
        </div>
